...
|-----------------|------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------|---------------------------|
| **EP01**        | Gestión de Reservas de Estacionamiento          | Como **visitante de la aplicación web de PARKINGNOW**, quiero gestionar mis reservas de estacionamiento, incluyendo la capacidad de hacer, modificar o cancelar reservas, para asegurarme de tener un espacio disponible cuando lo necesite. | N/A                     | N/A                       |
| **EP02**        | Información y Funcionalidades en el Landing Page | Como **visitante de la aplicación web de PARKINGNOW**, quiero acceder a información clara y utilizar funciones para buscar y reservar estacionamientos para entender cómo funciona PARKINGNOW y aprovechar sus beneficios. | N/A                     | N/A                       |
| **EP03**        | Integración RESTful API para la Gestión de Estacionamientos | Como **developer de PARKINGNOW**, quiero desarrollar e integrar un RESTful API para gestionar reservas, disponibilidad y notificaciones de estacionamientos de manera eficiente y segura. | N/A                     | N/A                       |
| **EP04**        | Monitoreo y Notificaciones de Disponibilidad en Tiempo Real | Como **developer de PARKINGNOW**, quiero implementar funcionalidades en el backend para monitorear la disponibilidad en tiempo real y enviar notificaciones a los usuarios sobre cambios en la disponibilidad de los espacios de estacionamiento. | N/A                     | N/A                       |
| **EP05**        | Gestión de Perfil y Preferencias de Usuario     | Como **visitante de la aplicación web de PARKINGNOW**, quiero gestionar mi perfil y preferencias (vehículo, método de pago) para personalizar mi experiencia de uso y facilitar futuras reservas. | N/A                     | N/A                       |
| **EP06**        | Seguridad y Autenticación en PARKINGNOW         | Como **developer de PARKINGNOW**, quiero asegurar que todas las operaciones en la plataforma (reserva, modificación, cancelación) estén protegidas mediante mecanismos de seguridad y autenticación adecuados. | N/A                     | N/A                       |
...
|---------------|--------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| **US01**      | Búsqueda de Estacionamiento                 | Como **visitante de la aplicación web de PARKINGNOW**, quiero buscar espacios de estacionamiento disponibles cerca de mi ubicación para estacionar rápidamente y sin problemas. | **Escenario 01**: Dado que un visitante necesita estacionar, **When** ingresa su ubicación en la aplicación web, **Then** se muestran los espacios disponibles dentro de un radio de 1 km.<br>**Escenario 02**: Dado que la disponibilidad cambia frecuentemente, **When** se libera un espacio cerca del visitante, **Then** el sistema envía una notificación de la nueva disponibilidad. | EP01                      |
| **US02**      | Reservar Espacio de Estacionamiento         | Como **visitante de la aplicación web de PARKINGNOW**, quiero reservar un espacio de estacionamiento para asegurarme de tener un lugar al llegar a mi destino. | **Escenario 01**: Dado que hay un espacio disponible, **When** el visitante selecciona "Reservar", **Then** el sistema confirma la reserva y envía una notificación de confirmación.<br>**Escenario 02**: Dado que un visitante intenta reservar un espacio ya reservado, **When** intenta confirmar la reserva, **Then** el sistema muestra un mensaje indicando que el espacio ya no está disponible. | EP01                      |
| **US03**      | Modificar Reserva                           | Como **visitante de la aplicación web de PARKINGNOW**, quiero modificar los detalles de mi reserva para ajustar el tiempo de llegada o salida según mis necesidades. | **Escenario 01**: Dado que una reserva está activa, **When** el visitante selecciona "Modificar reserva" y ajusta los detalles, **Then** el sistema actualiza la reserva y confirma la modificación.<br>**Escenario 02**: Dado que el visitante intenta modificar una reserva fuera del tiempo permitido, **When** selecciona "Modificar reserva", **Then** el sistema muestra un mensaje indicando que no se puede modificar. | EP01                      |
| **US04**      | Cancelar Reserva                            | Como **visitante de la aplicación web de PARKINGNOW**, quiero cancelar una reserva existente si mis planes cambian para evitar cargos innecesarios. | **Escenario 01**: Dado que una reserva está activa, **When** el visitante selecciona "Cancelar reserva", **Then** el sistema cancela la reserva y envía una confirmación de cancelación.<br>**Escenario 02**: Dado que el visitante intenta cancelar una reserva fuera del período de cancelación gratuito, **When** selecciona "Cancelar reserva", **Then** el sistema muestra los cargos aplicables. | EP01                      |
| **US05**      | Visualización de Servicios en el Landing Page | Como **visitante de la aplicación web de PARKINGNOW**, quiero ver una lista clara de los servicios ofrecidos por PARKINGNOW para entender cómo puede beneficiar mi experiencia de estacionamiento. | **Escenario 01**: Dado que el visitante está en la Landing Page, **When** navega a la sección de servicios, **Then** ve una lista detallada de todos los servicios con sus descripciones.<br>**Escenario 02**: Dado que el visitante está en la Landing Page, **When** selecciona un servicio específico, **Then** es dirigido a una página con más detalles sobre ese servicio. | EP02                      |
| **US06**      | Filtrar Resultados de Búsqueda              | Como **visitante de la aplicación web de PARKINGNOW**, quiero filtrar los resultados de búsqueda de estacionamiento por precio y distancia para encontrar la mejor opción para mí. | **Escenario 01**: Dado que el visitante está viendo los resultados de búsqueda, **When** selecciona filtros de precio y distancia, **Then** los resultados se actualizan según los criterios seleccionados.<br>**Escenario 02**: Dado que no hay resultados que coincidan con los filtros aplicados, **When** el visitante intenta filtrar, **Then** el sistema muestra un mensaje indicando que no hay coincidencias disponibles. | EP02                      |
| **US07**      | Crear Endpoint para Crear Reserva           | Como **developer de PARKINGNOW**, quiero crear un endpoint en la API para crear nuevas reservas de estacionamiento para que los usuarios puedan reservar desde cualquier dispositivo. | **Escenario 01**: Dado que una reserva necesita ser creada, **When** se envía una solicitud POST a /reservas con datos válidos, **Then** el sistema crea la reserva y devuelve una confirmación.<br>**Escenario 02**: Dado que los datos enviados son incompletos, **When** se envía una solicitud POST a /reservas, **Then** el sistema devuelve un error indicando los campos faltantes. | EP03                      |
| **US08**      | Crear Endpoint para Actualizar Reserva      | Como **developer de PARKINGNOW**, quiero crear un endpoint en la API para actualizar reservas existentes, para que los usuarios puedan modificar detalles de sus reservas. | **Escenario 01**: Dado que una reserva existente necesita ser actualizada, **When** se envía una solicitud PUT a /reservas/{id} con datos válidos, **Then** la reserva se actualiza correctamente y se devuelve una confirmación.<br>**Escenario 02**: Dado que se intenta actualizar una reserva inexistente, **When** se envía una solicitud PUT a /reservas/{id} con un ID inválido, **Then** el sistema devuelve un error indicando que la reserva no existe. | EP03                      |
| **US09**      | Crear Endpoint para Eliminar Reserva        | Como **developer de PARKINGNOW**, quiero crear un endpoint en la API para eliminar reservas cuando el usuario decide cancelar su espacio de estacionamiento. | **Escenario 01**: Dado que un usuario desea cancelar una reserva, **When** se envía una solicitud DELETE a /reservas/{id}, **Then** la reserva se elimina y se confirma la cancelación.<br>**Escenario 02**: Dado que se intenta eliminar una reserva ya cancelada, **When** se envía una solicitud DELETE a /reservas/{id}, **Then** el sistema devuelve un error indicando que la reserva ya fue cancelada. | EP03                      |
| **US10**      | Monitorear Disponibilidad en Tiempo Real    | Como **developer de PARKINGNOW**, quiero implementar un sistema que monitoree la disponibilidad de espacios en tiempo real y actualice la base de datos para reflejar los cambios de ocupación. | **Escenario 01**: Dado que un espacio de estacionamiento cambia de estado, **When** el sensor detecta el cambio, **Then** el sistema actualiza la disponibilidad en la base de datos.<br>**Escenario 02**: Dado que hay un fallo en la comunicación del sensor, **When** se intenta actualizar la disponibilidad, **Then** el sistema genera una alerta de error para revisión manual. | EP04                      |
| **US11**      | Notificar Cambios de Disponibilidad         | Como **developer de PARKINGNOW**, quiero enviar notificaciones automáticas a los usuarios cuando haya cambios en la disponibilidad de los espacios de estacionamiento. | **Escenario 01**: Dado que la disponibilidad de un espacio cambia, **When** el sistema detecta el cambio, **Then** se envía una notificación automática a los usuarios registrados interesados.<br>**Escenario 02**: Dado que un usuario no ha activado las notificaciones, **When** hay un cambio en la disponibilidad, **Then** el sistema no envía notificación a ese usuario. | EP04                      |
| **US12**      | Gestión de Preferencias de Notificación     | Como **visitante de la aplicación web de PARKINGNOW**, quiero gestionar mis preferencias de notificación para recibir alertas solo cuando me interesen cambios específicos en las reservas o disponibilidad. | **Escenario 01**: Dado que el visitante está en su perfil de usuario, **When** selecciona sus preferencias de notificación, **Then** el sistema guarda y aplica las configuraciones seleccionadas.<br>**Escenario 02**: Dado que el visitante no ha completado el formulario de preferencias, **When** intenta guardar los cambios, **Then** el sistema muestra un mensaje de error solicitando completar todos los campos. | EP05                      |
| **US13**      | Implementar Seguridad en API RESTful        | Como **developer de PARKINGNOW**, quiero asegurar que todas las solicitudes a la API RESTful estén autenticadas para proteger los datos de los usuarios y evitar accesos no autorizados. | **Escenario 01**: Dado que una solicitud se realiza a la API, **When** la solicitud incluye un token de autenticación válido, **Then** la solicitud se procesa correctamente.<br>**Escenario 02**: Dado que una solicitud no incluye un token válido, **When** se intenta acceder a la API, **Then** el sistema devuelve un error de autorización. | EP06                      |
| **US14**      | Verificar Información de Vehículos          | Como **visitante de la aplicación web de PARKINGNOW**, quiero poder agregar y verificar la información de mis vehículos en mi perfil para facilitar futuras reservas. | **Escenario 01**: Dado que el visitante está en su perfil, **When** añade un nuevo vehículo, **Then** el sistema guarda la información del vehículo y la muestra en el perfil.<br>**Escenario 02**: Dado que el visitante intenta agregar un vehículo con información incompleta, **When** selecciona "Guardar", **Then** el sistema muestra un mensaje indicando los campos obligatorios que faltan. | EP05                      |
| **US15**      | Recuperar Contraseña por Correo             | Como **visitante de la aplicación web de PARKINGNOW**, quiero poder recuperar mi contraseña fácilmente si la olvido para acceder nuevamente a mi cuenta sin complicaciones. | **Escenario 01**: Dado que el visitante ha olvidado su contraseña, **When** ingresa su correo electrónico y solicita la recuperación, **Then** el sistema envía un enlace de restablecimiento de contraseña al correo registrado.<br>**Escenario 02**: Dado que el visitante ingresa un correo electrónico no registrado, **When** intenta recuperar su contraseña, **Then** el sistema muestra un mensaje indicando que el correo no existe en los registros. | EP05                      |
| **US16**      | Notificación de Cambios en Reservas         | Como **visitante de la aplicación web de PARKINGNOW**, quiero recibir notificaciones automáticas si hay algún cambio en el estado de mi reserva para estar informado y tomar acciones si es necesario. | **Escenario 01**: Dado que una reserva ha sido realizada, **When** el estado de la reserva cambia, **Then** el sistema envía automáticamente una notificación al usuario.<br>**Escenario 02**: Dado que el visitante ha desactivado las notificaciones, **When** hay un cambio en el estado de la reserva, **Then** el sistema no envía notificación. | EP01                      |
| **US17**      | Confirmación de Cambios en Perfil           | Como **visitante de la aplicación web de PARKINGNOW**, quiero recibir una confirmación por correo electrónico cada vez que se realice un cambio en mi perfil para asegurarme de que mi información esté segura. | **Escenario 01**: Dado que un cambio en el perfil es realizado, **When** el visitante guarda los cambios, **Then** el sistema envía un correo electrónico de confirmación al correo registrado.<br>**Escenario 02**: Dado que el visitante no confirma el cambio con un código enviado, **When** intenta guardar un cambio crítico, **Then** el sistema solicita el código antes de proceder. | EP05                      |
| **US18**      | Soporte para Idiomas en la Plataforma       | Como **visitante de la aplicación web de PARKINGNOW**, quiero que la plataforma soporte múltiples idiomas para que sea más accesible a una audiencia internacional. | **Escenario 01**: Dado que el visitante selecciona un idioma preferido, **When** navega por la aplicación web, **Then** todo el contenido se muestra en el idioma seleccionado.<br>**Escenario 02**: Dado que el visitante cambia de idioma, **When** selecciona un nuevo idioma, **Then** la página se recarga mostrando el contenido en el idioma nuevo. | EP02                      |
| **US19**      | Notificaciones de Cambios de Precios        | Como **visitante de la aplicación web de PARKINGNOW**, quiero recibir notificaciones cuando haya cambios en los precios de estacionamiento para poder ajustar mi presupuesto. | **Escenario 01**: Dado que los precios de estacionamiento cambian, **When** el sistema detecta un cambio de precio, **Then** se envía una notificación automática a los usuarios interesados.<br>**Escenario 02**: Dado que el usuario ha seleccionado no recibir notificaciones de precios, **When** hay un cambio de precio, **Then** no se envía ninguna notificación a ese usuario. | EP04                      |
| **US20**      | Pruebas Automatizadas para API              | Como **developer de PARKINGNOW**, quiero implementar pruebas automatizadas para asegurar que todas las funcionalidades del API RESTful funcionen correctamente. | **Escenario 01**: Dado que una nueva funcionalidad se despliega, **When** se ejecutan las pruebas automatizadas, **Then** todas las pruebas deben pasar sin errores para confirmar la funcionalidad correcta.<br>**Escenario 02**: Dado que una prueba automatizada falla, **When** se revisa el log de errores, **Then** se identifican las causas y se realizan los ajustes necesarios. | EP03                      |
